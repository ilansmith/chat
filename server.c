...
static int s_mail_post(user_t *sender, user_t *receiver, tlv_t type)
{
...
#ifdef DEBUG
		PSVR(sender->name,
			"could not post a %s message to '%s' who is offline",
	...
	switch (type) {
		...
		/* in the following cases it is not necessary to send the
		 * name */
	...
	case COLLEGUE_REMOVE: /* user does not get notified                */
		...
		break;
		/* in this case the name must be sent for use after the
		 * unregistration */
	case FRIEND_UNREGISTER:
	case COLLEGUE_ADD:
		memcpy(buffer + sizeof(long), sender->name,
			MIN(strlen(sender->name), STR_MAX_NAME_LENGTH));
		mail_set_data(new_mail, type,
			sizeof(long) + MIN(strlen(sender->name), 
			STR_MAX_NAME_LENGTH), buffer);
		break;
	default:
#ifdef DEBUG
		PSVR(sender->name, "unknown type of message to be posted to " \
			"'%s'", receiver->name);
#endif
		mail_free(new_mail);
		return -1;
	 ...
	 * if a message already exists in the mailbox with the same sender id
	 * and  type, new_mail will be disregarded */
	last_mail = &(receiver->mailbox);
	while (*last_mail) {
		/* a sender cannot send the same message more than once
		 * before it gets handeld by the receiver */
		if ((MSG_TYP((*last_mail)->msg) == type) &&
			(*((long*)MSG_VAL((*last_mail)->msg)) ==
			 sender->cid)) {
			mail_free(new_mail);
#ifdef DEBUG
			PSVR(sender->name, "user '%s' already has a pending " \
				"%s message - did not resend", receiver->name,
				tlv_str[type]);
#endif
			return 0;
		}

		/* if there is a COLLEGUE_ADD followed by a COLLEGUE_REMOVE in
		 * the receiver's mailbox, both from the same sender, remove
		 * them */
		if ((type == COLLEGUE_REMOVE) && 
			...
			*last_mail = (*last_mail)->next;
			mail_free(new_mail);
#ifdef DEBUG
			PSVR(sender->name, "removed user '%s's pending " \
				"COLLEGUE_ADD message", receiver->name);
#endif
			return 0;
		}
...
{
	/* validate the the message gets handled in the current
	 * SVR_STATUS(svr) */
	...
	while (*mailptr) {
		...
		case COLLEGUE_ADD:
			...
			ASSERT(s_scollegue_insert(user, *((long*)(msg->value))),
				ERRT_PEERINSERT, ERRA_WARN, user->name,
				"could not add '%s' to list of collegues",
				user->name);
			mail_delete(mailptr);
			break;
			/* done at: logon */
		case FRIEND_UNREGISTER:
			...
			ASSERT(msg_send(msg, SVR_SCK(svr)), ERRT_SEND,
				ERRA_WARN, user->name,
				"could not send user message about the " \
				"logout of '%s'", (char*)MSG_VAL(msg));
			mail_delete(mailptr);
			break;
		default:
			mail_delete(mailptr);
...
static void s_sweep_collegues(svr_t *svr, user_t *user, tlv_t type)
{
	...
	while (collegue) {
		...
		ASSERT(s_mail_post(user, users[collegue->id], type),
			ERRT_POSTMSG, ERRA_WARN, user->name,
			"could not allocate a mail message to send to '%s'",
			users[collegue->id]->name);
	...
	case SSTAT_UNREGISTER:
		while (friend) {
			...
			ASSERT(success = s_mail_post(user, users[friend->id], 
				COLLEGUE_REMOVE), ERRT_POSTMSG, ERRA_WARN,
					user->name,  "could not send user " \
					"'%s' an unregister notification", 
...
static void s_command_add(svr_t *svr, char *name)
{
	...
	for (i = 0; i < usr_array_size; i++) {
		...
		ASSERT(success = s_mail_post(users[SVR_CID(svr)], users[i], 
			COLLEGUE_ADD), ERRT_POSTMSG, ERRA_WARN,
			users[SVR_CID(svr)]->name, "could not send user " \
			"'%s' an add collegue request", name);
		CS_END(users[i]->mutex);
		break;
	}
	...
	ASSERT(success = s_sfriend_insert(svr, i), ERRT_PEERINSERT, ERRA_WARN,
		users[SVR_CID(svr)]->name, "could not allocate entry for " \
		"new friend '%s'", name);
	...
	ASSERT(sz = msg_send(&out_msg, SVR_SCK(svr)), ERRT_SEND, ERRA_WARN,
		users[SVR_CID(svr)]->name, "could not send friend " \
		"initialization for %s", name);
...
Error:
	...
	ASSERT(msg_send(&out_msg, SVR_SCK(svr)), ERRT_SEND, ERRA_WARN,
		users[SVR_CID(svr)]->name, "could not add '%s' as a friend",
		name);
}
...
static void s_command_remove(svr_t *svr, long id)
{
	...
	ASSERT(s_mail_post(users[SVR_CID(svr)], users[id], COLLEGUE_REMOVE),
		ERRT_POSTMSG, ERRA_WARN, users[SVR_CID(svr)]->name,
		"could not send user '%s' a remove collegue request",
		users[id]->name);
...
static int s_mail_friend_status(svr_t *svr, msg_t *msg)
{
	...
	ASSERT(success = msg_send(msg, SVR_SCK(svr)), ERRT_SEND, ERRA_WARN,
		users[SVR_CID(svr)]->name, "could not send a friend %s " \
		"notification message to users",
		(*((tlv_t*)MSG_TYP(msg)) == FRIEND_LOGIN) ? "login" : "logout");
...
static int s_mail_friend_unregister(svr_t *svr, msg_t *msg)
{
	...
	s_sfriend_extract(svr, *((long*)MSG_VAL(msg)));
	ASSERT(success = msg_send(msg, SVR_SCK(svr)), ERRT_SEND, ERRA_WARN, 
		users[SVR_CID(svr)]->name, "could not sent a friend " \
...
static int s_mail_collegue_add(svr_t *svr, msg_t *msg)
{
	...
	/* notifying user */
	ASSERT(success = msg_send(msg, SVR_SCK(svr)), ERRT_SEND, ERRA_WARN,
		users[SVR_CID(svr)]->name, "could not notify user about new " \
		"collegue '%s'", collegue_name);

#ifdef DEBUG
	if (success != -1) {
		PSVR(users[SVR_CID(svr)]->name, "notifed user of new " \
			"collegue '%s'", collegue_name);
	}
#endif
	...
	ASSERT(success = s_mail_post(users[SVR_CID(svr)], users[collegue_id], 
		FRIEND_LOGIN), ERRT_POSTMSG, ERRA_WARN,
			users[SVR_CID(svr)]->name, "could not notify " \
			"collegue '%s' of user's status", collegue_name);
...
static void s_handle_mail(svr_t *svr)
{
	...
	*mailbox = (*mailbox)->next;
	...
	if (MSG_TYP(mail->msg) != IM) {
		PSVR(users[SVR_CID(svr)]->name, "received a %s message " \
			"from '%s'", tlv_str[MSG_TYP(mail->msg)],
	...
	case COLLEGUE_ADD:
		...
		break;
	case COLLEGUE_REMOVE:
		s_scollegue_extract(users[SVR_CID(svr)],
			*((long*)MSG_VAL(mail->msg)));
		break;
	default:
		/* TODO: sanity check */
		break;
...
#define TV_USEC 10
	...
	while (serve) {
		...
		case SSTAT_SERVE:
			...
			} while (!(request = select(SVR_SCK(svr) + 1,
				&rfds, NULL, NULL, &tv)) &&
				!(users[SVR_CID(svr)]->mailbox));
...
static tlv_t s_register(char *cname)
{
	...
	for (i = 0; i < usr_array_size; i++) {
		...
		/* verify that a user with the same name is not already
		 * registered */
		if (!memcmp(users[i]->name, cname, STR_MAX_NAME_LENGTH)) {
			first_free_index = -1;
			CS_END(mtx_usrcnt);
#ifdef DEBUG
			PSVR(STR_NIL, "register faild, user '%s' is already " \
				"registered", cname);
#endif
	...
	if (!(users[first_free_index] = user_alloc(cname, first_free_index))) {
		CS_END(mtx_usrcnt);
#ifdef DEBUG
		PSVR (STR_NIL, "memory allocation failure, user %s could not " \
			"be registered", cname);
#endif
		return REGISTER_FAIL_CAPACITY;
	}
...
static int s_unregister(svr_t *svr, char *cname)
{
	...
	for (i = 0; i < usr_array_size; i++) {
		...
		if (users[i]->connected) {
#ifdef DEBUG
			PSVR(STR_NIL, "unregister faild, user '%s' is " \
				"logged in", cname);
#endif
			CS_END(users[i]->mutex);
			return  -1;
...
static void s_init(svr_t *svr)
{
	...
	while (SVR_STATUS(svr) == SSTAT_INIT) {
		...
		case LOGIN:
			...
			memcpy(cname, (char*)MSG_VAL(&in_msg),
				MIN(MSG_LEN(&in_msg), STR_MAX_NAME_LENGTH));
			...
			case SSTAT_RELOGIN:
				...
				SVR_STATUS(svr) = SSTAT_INIT;
#ifdef DEBUG
				PSVR(STR_NIL, "login failed, user '%s' is " \
					"already logged in", cname);
#endif
				break;
			case SSTAT_INIT:
				msg_type = LOGIN_FAIL;
		...
		case REGISTER:
			...
			/* TODO: bug - same message for users[] full and
			 * re-register */
		...
		case UNREGISTER:
			...
			msg_type = s_unregister(svr, cname) ?
				UNREGISTER_FAIL : UNREGISTER_SUCCESS;
			break;
